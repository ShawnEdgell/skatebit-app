...
<div class="h-full">
  {#if loading}
    <div
      class="absolute inset-0 flex items-center justify-center z-10 bg-base-200/50"
    >
  ...
  {#if missingPath}
    ...
    <div
      class="flex flex-col h-full items-center justify-center text-center gap-4 p-4"
    >
    ...
    <ul>
      {#each entries as entry (entry?.path ?? Math.random())}
        {#if entry}
          <li>
            <div
              class="flex justify-between items-center w-full hover:bg-base-300 rounded-lg gap-2"
            >
              {#if entry.isDirectory}
                ...
                >
                  <span class="text-xl text-info group-hover:text-info-content"
                    >📁</span
                  >
                  ...
                  {#if entry.size != null}
                    <span
                      class="badge badge-xs badge-ghost flex-shrink-0 ml-auto mr-2"
                    >
              ...
              {:else}
                ...
                >
                  <span
                    class="text-xl text-base-content/80 group-hover:text-base-content"
                    >📄</span
                  >
                  <span class="truncate flex-1"
