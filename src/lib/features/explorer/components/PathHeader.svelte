<script lang="ts">
	...
	$: canGoBack = !!normalizePath(currentPath) && !!normalizePath(absoluteBasePath) && normalizePath(currentPath) !== normalizePath(absoluteBasePath);

	function handleSegmentClick(segment: PathSegment) {
		if (segment.isNavigable && segment.fullPath && normalizePath(segment.fullPath) !== normalizePath(currentPath)) {
...
<div class="flex items-center space-x-2 text-sm">
	...
	>
		<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M9 9l6-6m0 0l6 6m-6-6v12a6 6 0 01-12 0v-3" /></svg>
	</button>
	...
	<div class="flex items-center overflow-hidden z-10" aria-label="Breadcrumb">
		{#each pathSegments as segment, index (segment.fullPath + index)}
			{#if index > 0}<span class="mx-1 text-base-content/50 flex-shrink-0">/</span>{/if}
