...
<script lang="ts">
  ...
  import { modioSearchQuery, modioSearchResults, modioSortOrder } from '$lib/stores/modioSearchStore';
  ...
  async function handleSelectSort(e: CustomEvent<string>) {
    ...
    const scrollContainer = document.querySelector('#modio-map-list-container .scrollbar-thin'); // Add an ID to the MapList container div
  ...
  function handleLoadMore() {
    ...
    if (visibleCount < $modioSearchResults.length) {
      console.log(`Loading more mods... Current: ${visibleCount}, Total: ${$modioSearchResults.length}`);
