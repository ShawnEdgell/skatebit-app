...
<script lang="ts">
  import { onMount, onDestroy, createEventDispatcher, afterUpdate } from 'svelte'; // Add afterUpdate
  ...
  function setupObserver() {
    ...
    if (scrollContainer) { // Ensure scrollContainer exists
       observer = new IntersectionObserver(
         ...
         { root: scrollContainer, threshold: 0.1, rootMargin: '0px 0px 300px 0px' } // Increased rootMargin
       );
       if (sentinel) {
...
>
  ...
  {#if visibleCount < mods.length}
     <div bind:this={sentinel} class="flex-shrink-0 grid place-content-center p-4 min-w-[1px] h-full">
  ...
  {:else if !loading}
     ...
     <div class="flex-shrink-0 grid place-content-center p-4 min-w-[50px] h-full">
