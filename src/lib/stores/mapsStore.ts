...
export async function refreshLocalMaps() {
  ...
  console.log(
    `[${callId}] Attempting refreshLocalMaps... Current loading: ${get(localMapsLoading)}`,
  )

  if (get(localMapsLoading)) {
    console.log(`[${callId}] Local maps refresh already in progress. Skipping.`)
    return
  }
  ...
  try {
    ...
    if (!mapsDir || mapsDir.startsWith('/error') || mapsDir.trim() === '') {
      ...
      console.log(`[${callId}] Calling loadLocalMaps for directory: ${mapsDir}`)
...
export async function refreshModioMaps() {
  const callId = Date.now()
  console.log(
    `[${callId}] Attempting refreshModioMaps... Current loading: ${get(modioMapsLoading)}`,
  )
  ...
  try {
    ...
    console.log(`[${callId}] fetchAllMods returned. Found ${mods.length} mods.`)
    modioMaps.set(mods)
