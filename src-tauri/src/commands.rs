...
pub fn unzip_file(zip_path: String, maps_folder: String) -> Result<(), String> {
    ...
    for i in 0..archive.len() {
        ...
        if file.name().ends_with('/') || file.is_dir() {
            ...
            outfile
                .sync_all()
                .map_err(|e| format!("Failed sync file {:?}: {}", outpath, e))?;
        }
    }
...
pub fn save_file(path: String, contents: Vec<u8>) -> Result<(), String> {
    ...
    if let Some(parent) = file_path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed create dir structure {}: {}", path, e))?;
    }
...
pub async fn download_and_install(
    ...
    fs::create_dir_all(&full_maps_folder_path).map_err(|e| {
        format!(
            "Failed create destination dir {:?}: {}",
            full_maps_folder_path, e
        )
    })?;
...
pub fn list_local_maps(relative_maps_path: String) -> Result<Vec<FsEntry>, String> {
    ...
    println!(
        "[Rust CMD::list_local_maps] START - Path: {}",
    ...
    for entry_result in fs::read_dir(&maps_folder_path).map_err(|e| e.to_string())? {
        if let Ok(entry) = entry_result {
            ...
            if let Ok(metadata) = entry.metadata() {
                ...
                if name.is_none() && !is_directory {
                    skipped_count += 1;
                    continue;
                } else if name.is_none() && is_directory {
                    println!("WARN - Directory has NON-UTF8 name: {}", path.display());
                }
                ...
                if should_include {
                    ...
                    if final_entry.path.as_os_str().is_empty()
                        || (final_entry.name.is_none() && !final_entry.is_directory)
                    {
                        eprintln!(
                            "!!! INTERNAL SKIP !!! Path='{}', Name='{:?}'",
                            final_entry.path.display(),
                            final_entry.name
                        );
                        skipped_count += 1;
                        continue;
                    } else {
                        map_entries.push(final_entry);
                        processed_count += 1;
                    }
                } else {
                    skipped_count += 1;
                ...
                eprintln!(
                    "!!! ERROR !!! Could not get metadata: {}. Skipping.",
                    path.display()
                );
                error_count += 1;
            }
        } else {
            eprintln!("!!! ERROR !!! Could not read directory entry result. Skipping.");
            error_count += 1;
        }
    }
...
pub fn list_directory_entries(absolute_path: String) -> Result<Vec<FsEntry>, String> {
    ...
    println!(
        "[Rust CMD::list_directory_entries] START - Path: {}",
        dir_path.display()
    );
    ...
    for entry_result in fs::read_dir(&dir_path)
        .map_err(|e| format!("Failed read dir {}: {}", dir_path.display(), e))?
    {
        if let Ok(entry) = entry_result {
            ...
            if let Ok(metadata) = entry.metadata() {
                ...
                if path.as_os_str().is_empty() {
                    eprintln!("!!! SKIP entry with empty path");
                    continue;
                }
                if name.is_none() && !is_directory {
                    eprintln!("!!! SKIP file with invalid name: {}", path.display());
                    continue;
                }
                ...
                eprintln!("Warning: Could not get metadata {:?}", entry.path());
            }
        } else {
            eprintln!("Warning: Could not read directory entry.");
        }
    }
    entries.sort_by(|a, b| {
    ...
    println!(
        "[Rust CMD::list_directory_entries] END - Returning {} entries.",
        entries.len()
    );
    Ok(entries)
}
...
pub fn create_empty_file_rust(relative_path: String) -> Result<(), String> {
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed create parent directory '{}': {}", path.display(), e))?;
    }
    File::create(&path)
        .map(|_| ())
...
pub fn rename_fs_entry_rust(
    ...
    if !old_path.exists() {
        return Err(format!("Source '{}' not found.", old_path.display()));
    }
    if new_path.exists() {
...
pub fn delete_fs_entry_rust(relative_path: String) -> Result<(), String> {
    ...
    if path.is_dir() {
        fs::remove_dir_all(&path)
            .map_err(|e| format!("Failed delete directory '{}': {}", path.display(), e))
    } else if path.is_file() {
        fs::remove_file(&path)
            .map_err(|e| format!("Failed delete file '{}': {}", path.display(), e))
    } else {
        Err(format!("Cannot delete unknown type: {}", path.display()))
    }
}
