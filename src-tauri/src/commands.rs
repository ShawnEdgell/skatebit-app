...
pub async fn download_and_install(url: String, destination: String) -> Result<(), String> {
    // Retrieve the user's Documents directory using the directories crate.
    ...
    // Treat destination as a folder, e.g. "SkaterXL/Maps"
    ...
    // Clone the headers before consuming the response with .bytes()
    let headers = response.headers().clone();

    // Read the downloaded bytes (this consumes `response`).
    let bytes = response.bytes()
        .await
        .map_err(|e| format!("Failed to read response bytes: {}", e))?;

    // Check if the content is a ZIP file based on the ZIP signature.
    ...
    if is_zip {
        ...
        // For non-zip files, attempt to extract the filename from the headers.
